# Deploy — Patch Transition Runbook & Documentation
...
```bash
psql -f deploy/scripts/derive_phase_table.sql
```
...
```bash
psql -f deploy/scripts/verify_phase_table.sql
```
...
The analysis service extends the sibling project's Docker Compose. Add the analysis services to your main `docker-compose.yml`:

```yaml
# In your main docker-compose.yml (sibling project root)
include:
  - path: ../go-dota2-analysis/deploy/docker-compose.yml
```

Or merge the services directly.
...
Migrations are run via the sibling project's migrator. Apply `002_analytics.sql` from this project's `deploy/migrations/` directory.
...
deploy/
├── README.md                    # This file
├── docker-compose.yml           # Analysis service compose extension
...
├── migrations/
│   └── 002_analytics.sql        # Analytics schema, MVs, roles (run via sibling project)
├── models/
│   ├── imitation/
│   │   └── current/             # Active imitation learning model
│   └── value/                   # Value model directory
├── scripts/
│   ├── derive_phase_table.sql   # Extract empirical phase table
│   └── verify_phase_table.sql   # Validate phase table
└── ...
```
...
2. **Verify artifacts** were produced:
   ```bash
   ls -la deploy/models/imitation/current/
...
cp /path/to/new/model.bin deploy/models/imitation/current/
cp /path/to/new/spec.json deploy/models/imitation/current/
cp /path/to/new/meta.json deploy/models/imitation/current/
